...
def pegar_texto():
    ...
    texto_paste = scrolledtext.ScrolledText(ventana_paste, width=40, height=20, wrap=tk.WORD,fg="white",cursor="hand2",relief="flat",bg=fondo)
    ...
    btn_analizar = tk.Button(ventana_paste, text="Analizar Texto", command=analizar_texto_pegado,fg="white",cursor="hand2",relief="flat",bg=fondo_correcto,font=("Comic Snas Ms", 8,"bold"))
    btn_analizar.pack()
...
def abrir_ventana_correcta():
    ...
    global iniciar_detener_button
    iniciar_detener_button = tk.Button(ventana_transcripcion, text="Iniciar Grabación", command=iniciar_grabacion,fg="white",cursor="hand2",relief="flat",bg=fondo_correcto,font=("Comic Snas Ms", 8,"bold"))
    iniciar_detener_button.pack()
    iniciar_detener_button.place(x=434, y=80)

    # Crear cuadro de texto para mostrar el resultado
    global resultado_text
    resultado_text = tk.Text(ventana_transcripcion, height=20, width=42,fg="white",cursor="hand2",relief="flat",bg=fondo)
    resultado_text.pack()
    resultado_text.place(x=40, y=45)

    # Crear botón "Volver a Grabar"
    global volver_grabar_button
    volver_grabar_button = tk.Button(ventana_transcripcion, text="Volver a Grabar", command=volver_grabar, state=tk.DISABLED,fg="white",cursor="hand2",relief="flat",bg=fondo_correcto,font=("Comic Snas Ms", 8,"bold"))
    volver_grabar_button.pack()
    volver_grabar_button.place(x=437, y=150)

    # Crear botón "Guardar"
    global Guardar_button
    Guardar_button = tk.Button(ventana_transcripcion, text="Guardar", state=tk.DISABLED,fg="white",cursor="hand2",relief="flat",bg=fondo_correcto,font=("Comic Snas Ms", 8,"bold"))
    Guardar_button.pack()
    Guardar_button.place(x=455, y=227)

    # Crear etiqueta para mostrar el temporizador
    global timer_label
    timer_label = tk.Label(ventana_transcripcion, text="Duración de la grabación: 0 segundos",fg="white",bg=fondo_ventana)
    timer_label.pack()
    timer_label.place(x=40, y=370)

    # Botón para pegar texto
    btn_valorar = tk.Button(ventana_transcripcion, text="Valorar", command=pegar_texto,fg="white",cursor="hand2",relief="flat",bg=fondo_correcto,font=("Comic Snas Ms", 8,"bold"))
    btn_valorar.pack()
...
def abrir_ventana_incorrecta():
    ...
    # Botón "Regresar"
    boton_regresar = tk.Button(ventana_error, text="Regresar", command=regresar_pagina_principal,fg="white",cursor="hand2",relief="flat",bg=fondo_incorrecto,font=("Bahnschrift Light",11,"bold"))
    boton_regresar.pack()
...
def login():
    ...
    if nombre == "AFED" and contrasenia == "1234":
...
entrada_usuario = tk.Entry(ventana_login, textvar=usuario, width=30, relief="flat",fg="white", bg=fondo_entrada,font=("Bahnschrift Light",11))
entrada_usuario.pack()
entrada_usuario.place(x=148, y=176)

# Etiqueta y entrada para la contraseña
entrada_password = tk.Entry(ventana_login, textvar=password, show="*", width=30, relief="flat", fg="white",bg=fondo_entrada,font=("Bahnschrift Light",11))
entrada_password.pack()
entrada_password.place(x=148, y=263)

def salir():
    ventana_login.destroy()

# Botón para iniciar sesión
boton_login = tk.Button(ventana_login, text="Entrar", command=login, cursor="hand2", bg=fondo_entrar,fg="white", width=10, relief="flat", font=("Bahnschrift Light",11,"bold"))
boton_login.pack()
boton_login.place(x=94, y=362)
boton1=tk.Button(ventana_login,text="Salir",command=salir ,cursor="hand2",bg=fondo_salir,fg="white",width=10,relief="flat", font=("Bahnschrift Light",11,"bold"))
